  ...
  const tbody = () => {
    ...
    return LeaveData.map((branch) => ({
      ...
      { data: branch.name },
        ...
        { data: branch.LeaveDays },
        { data: branch.FromDate },
        { data: branch.ToDate },

        {
          data: (
            <div
              className={`px-3 py-1 rounded-full text-sm inline-flex items-center ${branch.status === "apporved" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
            >
              <span
                className={`w-2 h-2 rounded-full mr-2 ${branch.status === "apporved" ? "bg-green-800" : "bg-red-700"}`}
              ></span>
