...
const location = useLocation();
  ...
  // Get full path → e.g. "/project/02-sp2"
  const pathname = location.pathname;

  // Split and take last part → "02-sp2"
  const project_code = pathname.split("/").pop();

  console.log("Current Project Code:", project_code);
...
if (hydrated && token) {
  ...
  const getBranch = async (
    ...
    try {
      ...
      setSheetData(response?.data?.revisions || []);
setTotalItem(response?.data?.count || 0);
setData(response?.data?.revisions || []);
  ...
  const columns = [
    ...
    { key: "milestitle", label: "Milestone Name" },
    // { key: "client_code", label: "Client Code" },
    { key: "previous_total_amount", label: "Previous Base Amount" },
    { key: "revision_reason", label: "Revision Reason" },
    { key: "base_amount", label: "Revised Base Amount" },
    { key: "additional_amount", label: "Revised Additional Amount" },
    { key: "revised_amount", label: "Revised Total Amount" },
    { key: "revised_difference", label: "Difference on Revision" },
    { key: "revised_at", label: "Revised at" },
    ...
    return processedData.map((project, index) => {
      ...
      return {
        data: [
          ...
          { data: project.miles_title },
          { data: project.previous_total_amount },
          { data: project.revision_reason },
          { data: project.base_amount },
          { data: project.additional_amount },
          { data: project.total_amount },
          { data: (project.total_amount-project.previous_total_amount) },
          { data: project.created_on },
          ,
        ].map((col, idx) => ({
          ...col,
          data:
