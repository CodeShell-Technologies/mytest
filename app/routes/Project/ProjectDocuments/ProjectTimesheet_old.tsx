...
const ProjectTimesheet = () => {
  ...
  const fetchTaskData = async () => {
    try {
      const token = localStorage.getItem('token'); //  Adjust as per your storage

      const response = await axios.get(
        'http://localhost:3000/api/project/task/checkinout/read',
        {
          headers: {
      ...
      const transformed = rawData.map((item) => ({
        ...
        department: item.department_name || "N/A", // ✅ If available
        priority:
      ...
      setData(transformed);
      setSheetData(transformed); // ✅ Set sheet data directly here
    } catch (error) {
...
const tbody = () => {
  ...
  return data.map((user) => ({
    ...
    data: [
      ...
      { data: user.date },
      { data: user.branch },
      { data: user.department }, // 👈 Department column added here
      { data: user.priority },
      {
        data: (
          <div className="flex">
