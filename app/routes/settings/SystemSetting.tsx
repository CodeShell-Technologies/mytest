...
import LeaveTy from "./leave/LeaveTy";
const SystemSetting = () => {
  ...
  const getSoftware = async (
    ...
    setLoading(true);
    try {
      let url = `${BASE_URL}/master/software/read?branchcode=${branchCode}&page=${page}&limit=${limit}`;
  ...
  const handleCreateSuccess = () => {
    ...
    toast.success("Software added successfully!", toastStyle);
    fetchBranches(token);
    getSoftware();
  };
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${BASE_URL}/master/software/delete/${id}`,
        {
      ...
      if (response?.status === 201) {
        ...
        toast.success("Software deleted successfully!",toastStyle);
        fetchBranches(token);
        getSoftware();
      }
    } catch (error) {
  ...
  const handleEditSuccess = () => {
    ...
    toast.success("Branch updated successfully!",toastStyle);
    fetchBranches(token);
    getSoftware();
  };
